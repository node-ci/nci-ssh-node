...
module.exports = function(app) {
	...
	Command.prototype.run = function(params, callback) {
		...
		var remoteCmd = this.shell;

		if (self.shellExtraArgs.length) {
			remoteCmd +=  ' ' + _(self.shellExtraArgs).map(function(arg) {
				return '"' + self._escapeArg(arg) + '"';
			}).join(' ');
		}

		remoteCmd += ' ' + this.shellCmdArg + ' ';

		// start shell command argument
		remoteCmd += '\'';

		var envVarsString = _(params.envVars).map(function(value, name) {
			return 'export ' + name + '="' + value + '"';
		}).join('; ');

		if (envVarsString) {
			remoteCmd += envVarsString + '; ';
		}
